...
async function appendToSheet(row: string[]) {
  ...
  if (!email || !key) {
    console.log("Google Sheets logging skipped — service account not configured");
    return;
  }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { variant, name, firstName, lastName, email, phone, subject, summary, lang } = req.body;
  ...
  try {
    ...
    // 3. Log to Google Sheets (non-blocking, won't fail the request)
    ...
    appendToSheet(sheetRow).catch((err) => console.error("Sheet log error:", err));
